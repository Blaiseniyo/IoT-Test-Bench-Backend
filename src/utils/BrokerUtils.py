...
class Broker:
    run_flag = True
    messages_received = 0
    results = {"sentMessages": 0, "recievedMessages": 0, "sentMessages_percentage": 0, "failedMessage": 0,
        ...
        if connected:
            ...
            for subscriber in range(body["subscribers"]):
                client = self.connect_mqtt(
                    self, broker=body['host'], port=body["port"], username=body["username"], password=body["password"], num=subscriber)
                client.loop_start()
            ...
            for publisher in range(body["publishers"]):
                client = self.connect_mqtt(
                    self, broker=body['host'], port=body["port"], username=body["username"], password=body["password"], num=publisher)
                client.loop_start()
                self.publish(
            ...
            self.results["messages_per_second"] = int(
                self.results["totalTime"]/msg_count)
            self.results["received_per_second"] = int(
                self.results["recievedMessages"]/self.results["totalTime"])
            self.results["recievedSize"] = self.results["recievedMessages"] * \
                body["message_size"]
            self.results["sentSize"] = self.results["sentMessages"] * \
                body["message_size"]
            self.results["sentMessages_percentage"] = int(
                ((self.results["recievedMessages"]/body["subscribers"]) / self.results["sentMessages"]) * 100)
            self.results["CPU_used"] = psutil.cpu_percent(
                int(time.time()-program_start))
            self.results["RAM_percent_used"] = psutil.virtual_memory()[2]
            self.results["RAM_size_used"] = int(
